function tranjection(convertAmmountInput,name){
    const li=document.createElement('li');
    li.innerHTML=`

    <div class="mt-4 flex items-center justify-between p-2 bg-white rounded-2xl shadow-sm w-full max-w-md">
    <div class="flex items-center">
      <div class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
        <div class="relative">
          <img src="./assets/wallet1.png" alt="icon" class="w-8 h-8"/>
        </div>
      </div>
      <div class="ml-4">
        <h2 class="text-lg font-semibold text-gray-800">${name}</h2>
        <p class="text-sm text-gray-500 font-bold">${convertAmmountInput}Tk</p>
        <p class="text-sm text-gray-500">Today 01:44 AM</p>
      </div>
    </div>
    <div>
